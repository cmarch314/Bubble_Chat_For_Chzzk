...
function playZergSound(fileName) {
    if (!soundEnabled) return; // [Config] Check Sound Toggle
    const audio = new Audio(fileName);
    audio.volume = 0.5; // 너무 크면 테란들이 놀라니 50%로 고정
    audio.play().catch(e => console.error("음향 소환 실패:", e));
}
function showMessage({ chan, type, message = '', data = {}, timeout = 6500, attribs = {} } = {}) {
    ...
    if (message.includes("해골")) {
        triggerSkullEffect();
    } else if (message.includes("커플")) {
        triggerHeartEffect();
    }

    Object.keys(soundHive).forEach(keyword => {
        if (message.includes(keyword)) {
            // "파쏘 뱀 쏠토" is handled above, so we don't need to check it here if it's not in keys
            // But if user adds it back, we should prevent double trigger logic if needed.
            // Since user renamed it to '해골', it won't match here anyway.
