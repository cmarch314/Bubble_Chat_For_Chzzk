...
const runtimeOutputPath = path.join(projectRoot, 'js', 'audio-gains.runtime.js');
...
function writeOutput(entries) {
    ...
    const runtimeGains = Object.fromEntries(
        Object.entries(ordered).map(([key, entry]) => [key, Number(entry.gainDb) || 0])
    );
    const source = [
    ...
    const runtimeSource = [
        '// Generated runtime projection. Full measurement evidence stays in audio-levels.generated.js.',
        `window.HIVE_AUDIO_GAINS=${JSON.stringify(runtimeGains)};`,
        ''
    ].join('\n');
    fs.writeFileSync(outputPath, source, 'utf8');
    fs.writeFileSync(runtimeOutputPath, runtimeSource, 'utf8');
}
...
async function main() {
    ...
    const activeKeys = new Set(files.map(filePath => relativePath(filePath)));
