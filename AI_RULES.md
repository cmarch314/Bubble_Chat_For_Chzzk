...
### 8-1. 사용자 승인 필요성 안내
- 터미널 명령어나 파일 쓰기 등 사용자 승인이 필요한 도구를 호출할 경우, **반드시 사용자에게 어떤 작업을 하려는지 설명하고 승인을 요청하는 텍스트 메시지를 출력**하세요.
- 도구를 호출한 뒤 텍스트 응답 없이 대기하면, 사용자가 승인 버튼이 뜬 것을 모르거나 서로 대기하게 되어 **무한 대기(Deadlock) 상태**에 빠질 수 있습니다.

### 8-2. 비동기 작업에 대한 타이머(Timer) 등록
- 빌드, 테스트, 또는 서브에이전트(subagent) 호출 등 시간이 오래 걸리거나 비동기로 실행되는 작업을 남겨두고 턴을 넘길(대기할) 때는 **반드시 `schedule` 도구를 사용하여 적절한 시간의 알림 타이머를 등록**하세요.
- 백그라운드 작업이나 서버가 조용히 멈추거나 재시작되어 알림이 오지 않는 문제를 예방하여, 타이머가 만료되었을 때 깨어나 상태를 재점검할 수 있습니다.

### 8-3. 세션 재시작/재개 시 상태 점검
