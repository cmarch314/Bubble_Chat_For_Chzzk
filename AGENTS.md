...
- `HuntBeatV2Contract`/`HuntHunterBeatCatalog` own authored graphs; `HuntBeatActionRuntime` advances them. `HuntCombatJudgmentRuntime` traces and `HuntCombatJudgmentResolver` alone mutates combat. Approved BEAT never uses legacy impact/renderer timing.
- `HuntRotationContract` exclusively owns image-local clockwise/counterclockwise resolution, accumulated angles, and recovery preservation. The editor and `HuntMotionCompiler` consume it directly; facing mirrors reverse the visible path once, and home translation never authors a recovery turn.
...
- `HuntRotationContract` owns image-local direction, accumulated angles, and reset modes. Candidate/editor input removes competing absolute angles and rejects `rotateBy*`/`keepRotation`; Preview uses the current draft. Home translation preserves rotation until completion, never reversing toward `rotate(0)`.
