...
- Preview-authored damage judgments own hit timing, target, damage, reaction kind, and recovery; the generated runtime graph carries them unchanged into `HuntEngine.presentHunterImpact`. Profile tags are legacy fallback only. Renderers simulate only behind preview flags; recovery tests pin ownership/idempotency.
