...
- `MonsterAudioReviewState.EditorSession` owns review-editor pattern, timeline, selection, scrub, scenario, history, and dirty state. `MonsterMotionAuthoringContract` owns editable fields, normalization, storage, and round-trip comparison across motion sources; save paths must not keep private allowlists. The v3 review server serves the editor and embedded real-hunt preview from one origin, rejects stale revisions, and reload-verifies generated data before reporting a save. Live simulation boots production `index.html` and drives its real chat router; never add a preview-only battle loop.
