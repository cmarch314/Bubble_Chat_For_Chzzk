...
- `HuntBeatV2Contract`/`HuntHunterBeatCatalog` own authored graphs; `HuntBeatActionRuntime` advances them. `HuntProjectilePresentationRuntime` owns projectile identity, launch/outcome/finish lifecycle and has no clock of its own. `HuntCombatJudgmentRuntime` traces and `HuntCombatJudgmentResolver` alone mutates combat. Approved BEAT never uses legacy impact/renderer timing.
...
- `HuntCombatRuntime` alone advances `HuntEngine`; `HuntCombatClock` provides automatic live or manual Preview transport. Preview submits actions here and never schedules duplicate motion, judgment, or audio.
...
- The motion-only editor renders the authored BEAT through `HuntMonsterAttackAnimator.playBeatMotion`; it keeps target geometry for placement but never constructs `HuntCombatRuntime` or resolves hits. Scrubbing seeks that animation; live hunt owns combat.
...
- Monster motion has one graph contract: authored `beat` or extracted `keyframe-beat`. `judgments[]` alone owns contact timing and targets; projectile `projectile-launch` / `projectile-finish` events independently own launch and cleanup, and the editor must round-trip and display all three lifecycle points without moving one as a side effect of another. Multi-target beats encode left/right in each judgment, never only outer targeting metadata. Never revive CSS capture, `hitOffsetTicks`, `judgmentOffsets`, inferred projectile delays, or phase-slot audio ownership.
- Preview/editor and live compile the same pattern, targets, and BEAT clock. Judgments own each hit; recovery clears its visual lock so later hits remain valid. Never mix edited motion with saved `beatV2`/impact data or invent Preview-only hits/audio; parity tests enforce this.
