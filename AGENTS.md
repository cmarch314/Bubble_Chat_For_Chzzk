...
- `HuntBeatV2Contract`/`HuntHunterBeatCatalog` own graphs; `HuntBeatActionRuntime` advances them. `HuntProjectileTimingResolver` resolves distance before sessions; `HuntProjectilePresentationRuntime` presents lifecycle without a clock. `HuntCombatJudgmentRuntime` traces and `HuntCombatJudgmentResolver` mutates combat. Approved BEAT never uses legacy timing.
