...
- `HuntEngine.presentHunterImpact` exclusively emits committed live hit/guard/dodge reactions; monster motion renderers may simulate them only behind explicit preview flags. `pending`, `invulnerable`, miss, resist, and effect-only judgments stay silent; focused recovery tests pin ownership and renderer idempotency.
