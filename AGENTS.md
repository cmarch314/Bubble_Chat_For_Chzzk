...
- `MonsterAudioReviewState.EditorSession` is the sole owner of review-editor pattern, timeline, selection, scrub, scenario, history, and dirty state. The v3 review server serves the editor and embedded real-hunt preview from one origin, rejects stale source revisions, and reload-verifies generated data before reporting a save. Live simulation boots production `index.html` and drives its real chat router; never add a preview-only battle loop.
...
- The embedded pattern editor is motion-only: it renders the same authored monster BEAT graph through `HuntMonsterAttackAnimator.playBeatMotion`, preserves target geometry only for route placement, and never constructs `HuntCombatRuntime` or resolves hunter damage/reactions. Scrubbing pauses/seeks that shared animation directly; live hunt remains the sole owner of hit resolution, card reactions, and combat timers.
