class HuntAtbConfig {
    ...
    static monsterActionCostGauge(pattern = {}, timing = {}) {
        ...
        // ATB recovers during telegraphs, travel and animation locks. Charge for
        // that recovered amount as action debt, then leave a class-sized gap
        // after the visible motion. Without this floor, a long animation reaches
        // a full gauge before it ends and the monster can immediately overlap
        // another attack.
        ...
        const recoveredDuringAction = occupancyTicks * recoveryPerTick;
        const postActionSeconds = Number(
            this.MONSTER_POST_ACTION_RECOVERY_SECONDS[actionClass]
            ?? this.MONSTER_POST_ACTION_RECOVERY_SECONDS.normal
        );
        const cadenceFloor = occupancyTicks > 0
            ? recoveredDuringAction + postActionSeconds * this.GAUGE_PER_SECOND * multiplier
            : 0;
        const maxDebt = this.GAUGE_MAX * this.MAX_MONSTER_ACTION_DEBT_GAUGES;
        return Math.round(Math.max(0, Math.min(maxDebt, Math.max(authoredCost, cadenceFloor))) * 1000000) / 1000000;
    }

    static monsterAtbAfterPartBreak(currentAtb) {
        const visibleGauge = Math.max(0, Math.min(this.GAUGE_MAX, Number(currentAtb || 0)));
        return visibleGauge * this.MONSTER_PART_BREAK_ATB_RETAIN_RATE;
    }
