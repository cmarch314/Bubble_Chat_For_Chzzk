class VergilEffect extends BaseEffect {
    ...
    async execute(context) {
        ...
        return new Promise(resolve => {
            this.timers.timeout(() => { if (ov.parentNode) ov.remove(); resolve(); }, conf.duration);

            const slashTrembleTime = conf.slashTrembleTime || 5200;
            const explosionTime = conf.explosionTime || 6200;

            this.timers.timeout(() => { slashes.forEach(s => s.style.animation = `hvn-vergil-slashTremble ${0.05 + Math.random() * 0.1}s infinite`); }, slashTrembleTime);

            this.timers.timeout(() => {
                slashes.forEach(s => { s.style.animation = "hvn-vergil-fadeOut 1s forwards"; this.timers.timeout(() => s.remove(), 1000); });
                for (let i = 0; i < 12; i++) {
                    ...
                    for (let j = 0; j < conf.shardCount; j++) {
                        ...
                        ov.appendChild(shard); this.timers.timeout(() => shard.remove(), dur * 1000);
                    }
                }
                ...
                this.timers.timeout(() => {
                    ...
                    if (msg) {
                        ...
                        this.timers.timeout(() => { txt.style.animation = "hvn-vergil-fadeOut 1s forwards"; this.timers.timeout(() => txt.remove(), 1000); }, 7000);
                    }
